package org.epam.stepDefinition.UI;
...
public class WorkoutStep {
    ...
    @Then("the feedback button should not be active and feedback could not be submitted")
    public void theFeedbackShouldNotBeSubmitted() {
        boolean isFeedbackButtonEnabled = workoutPage.isSubmitFeedbackButtonEnabled();
        log.info("Feedback status displayed: {}", isFeedbackButtonEnabled);
        assertFalse(isFeedbackButtonEnabled);

    }

    @And("I give the desired feedback {string}")
    public void iGiveTheDesiredFeedback(String feedback) {
        workoutPage.setFeedbackTextArea(feedback);
    }
}
