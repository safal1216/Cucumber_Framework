...
public class WorkoutPage extends BasePage{
    ...
    @FindBy(xpath = "//button[text()='Submit Feedback']")
    private WebElement submitFeedbackPopUpButtonCheck;
    ...
    public boolean isSubmitFeedbackButtonEnabled() {
        try {
            wait.until(ExpectedConditions.visibilityOf(submitFeedbackPopUpButtonCheck));
            return submitFeedbackPopUpButtonCheck.isEnabled();
        } catch (Exception e) {
            return false;
        }
    }
